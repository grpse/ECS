...
public:
    explicit PoolAllocator(size_t poolsize) {
        ...
        mShouldFreePool = true;
    }

    explicit PoolAllocator(byte* preAllocatedBlock, size_t blockSize) {
        mPoolIndex = 0;
        mPoolSize = blockSize;
        mPool = preAllocatedBlock;
        mFreeList.reserve(mPoolSize);
        mShouldFreePool = false;
    }
...
private:
    ...
    // ugly constructor call syntax
    template<typename Type>
    void construct(Type* object_ptr) {
        ::new((void*)object_ptr) Type();
    }
};
